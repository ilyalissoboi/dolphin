...
AchievementSettingsWidget::AchievementSettingsWidget(QWidget* parent)
    : QWidget(parent), m_ui(std::make_unique<Ui::AchievementSettingsWidget>())
{
  m_ui->setupUi(this);
#ifndef USE_DISCORD_PRESENCE
  m_ui->common_discord_presence_enabled_input->setVisible(false);
#endif
  SetDescriptions();
  LoadSettings();
  ConnectWidgets();
...
void AchievementSettingsWidget::UpdateData(int login_failed_code)
{
  if (login_failed_code != RC_OK)
  {
    ...
    case RC_EXPIRED_TOKEN:
      m_ui->common_login_failed->setText(tr("Logged Out - Credentials Expired"));
      break;
    case RC_INVALID_CREDENTIALS:
      m_ui->common_login_failed->setText(tr("Login Failed - Invalid Username/Password"));
      break;
    case RC_NO_RESPONSE:
      m_ui->common_login_failed->setText(tr("Login Failed - No Internet Connection"));
      break;
    default:
      m_ui->common_login_failed->setText(tr("Login Failed - Server Error"));
      break;
    }
    m_ui->common_login_failed->setVisible(true);
  }
  LoadSettings();
}

void AchievementSettingsWidget::SetDescriptions()
{
  ConfigWidget::SetDescription(
      m_ui->common_integration_enabled_input, {},
  ...
  ConfigWidget::SetDescription(
      m_ui->common_hardcore_enabled_input, {},
      tr("Enable Hardcore Mode on RetroAchievements.<br><br>Hardcore Mode is intended to provide "
         ...
         "re-enabling."));
  ConfigWidget::SetDescription(
      m_ui->common_unofficial_enabled_input, {},
  ...
  ConfigWidget::SetDescription(
      m_ui->common_encore_enabled_input, {},
  ...
  ConfigWidget::SetDescription(
      m_ui->common_spectator_enabled_input, {},
  ...
  ConfigWidget::SetDescription(
      m_ui->common_leaderboard_tracker_enabled_input, {},
  ...
  ConfigWidget::SetDescription(
      m_ui->common_challenge_indicators_enabled_input, {},
...
{
  connect(m_ui->common_integration_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleRAIntegration);
  connect(m_ui->common_login_button, &QPushButton::clicked, this,
          &AchievementSettingsWidget::Login);
  connect(m_ui->common_logout_button, &QPushButton::clicked, this,
          &AchievementSettingsWidget::Logout);
  connect(m_ui->common_hardcore_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleHardcore);
  connect(m_ui->common_unofficial_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleUnofficial);
  connect(m_ui->common_encore_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleEncore);
  connect(m_ui->common_spectator_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleSpectator);
  connect(m_ui->common_leaderboard_tracker_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleLeaderboardTracker);
  connect(m_ui->common_challenge_indicators_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleChallengeIndicators);
  connect(m_ui->common_discord_presence_enabled_input, &QCheckBox::toggled, this,
          &AchievementSettingsWidget::ToggleDiscordPresence);
  connect(m_ui->common_progress_enabled_input, &QCheckBox::toggled, this,
...
void AchievementSettingsWidget::LoadSettings()
{
  ...
  SignalBlocking(m_ui->common_integration_enabled_input)->setChecked(enabled);
  SignalBlocking(m_ui->common_username_label)->setEnabled(enabled);
  if (!username.empty())
    SignalBlocking(m_ui->common_username_input)->setText(QString::fromStdString(username));
  SignalBlocking(m_ui->common_username_input)->setEnabled(enabled && logged_out);
  SignalBlocking(m_ui->common_password_label)->setVisible(logged_out);
  SignalBlocking(m_ui->common_password_label)->setEnabled(enabled);
  SignalBlocking(m_ui->common_password_input)->setVisible(logged_out);
  SignalBlocking(m_ui->common_password_input)->setEnabled(enabled);
  SignalBlocking(m_ui->common_login_button)->setVisible(logged_out);
  SignalBlocking(m_ui->common_login_button)->setEnabled(enabled && Core::IsUninitialized(system));
  if (!enabled || Core::IsUninitialized(system))
    SignalBlocking(m_ui->common_login_button)->setText(tr("Log In"));
  else
    SignalBlocking(m_ui->common_login_button)
        ->setText(tr("To log in, stop the current emulation."));

  SignalBlocking(m_ui->common_logout_button)->setVisible(!logged_out);
  SignalBlocking(m_ui->common_logout_button)->setEnabled(enabled);

  SignalBlocking(m_ui->common_hardcore_enabled_input)
  ...
  SignalBlocking(m_ui->common_unofficial_enabled_input)
      ->setChecked(Config::Get(Config::RA_UNOFFICIAL_ENABLED));
  SignalBlocking(m_ui->common_unofficial_enabled_input)->setEnabled(enabled);

  SignalBlocking(m_ui->common_encore_enabled_input)
      ->setChecked(Config::Get(Config::RA_ENCORE_ENABLED));
  SignalBlocking(m_ui->common_encore_enabled_input)->setEnabled(enabled);

  SignalBlocking(m_ui->common_spectator_enabled_input)
      ->setChecked(Config::Get(Config::RA_SPECTATOR_ENABLED));
  SignalBlocking(m_ui->common_spectator_enabled_input)->setEnabled(enabled);

  SignalBlocking(m_ui->common_leaderboard_tracker_enabled_input)
      ->setChecked(Config::Get(Config::RA_LEADERBOARD_TRACKER_ENABLED));
  SignalBlocking(m_ui->common_leaderboard_tracker_enabled_input)->setEnabled(enabled);

  SignalBlocking(m_ui->common_challenge_indicators_enabled_input)
      ->setChecked(Config::Get(Config::RA_CHALLENGE_INDICATORS_ENABLED));
  SignalBlocking(m_ui->common_challenge_indicators_enabled_input)->setEnabled(enabled);

  SignalBlocking(m_ui->common_discord_presence_enabled_input)
      ->setChecked(Config::Get(Config::RA_DISCORD_PRESENCE_ENABLED));
  SignalBlocking(m_ui->common_discord_presence_enabled_input)
      ->setEnabled(enabled && Config::Get(Config::MAIN_USE_DISCORD_PRESENCE));

  SignalBlocking(m_ui->common_progress_enabled_input)
      ->setChecked(Config::Get(Config::RA_PROGRESS_ENABLED));
  SignalBlocking(m_ui->common_progress_enabled_input)->setEnabled(enabled);
}
...
void AchievementSettingsWidget::SaveSettings()
{
  ...
  Config::SetBaseOrCurrent(Config::RA_ENABLED, m_ui->common_integration_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_HARDCORE_ENABLED,
                           m_ui->common_hardcore_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_UNOFFICIAL_ENABLED,
                           m_ui->common_unofficial_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_ENCORE_ENABLED,
                           m_ui->common_encore_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_SPECTATOR_ENABLED,
                           m_ui->common_spectator_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_LEADERBOARD_TRACKER_ENABLED,
                           m_ui->common_leaderboard_tracker_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_CHALLENGE_INDICATORS_ENABLED,
                           m_ui->common_challenge_indicators_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_DISCORD_PRESENCE_ENABLED,
                           m_ui->common_discord_presence_enabled_input->isChecked());
  Config::SetBaseOrCurrent(Config::RA_PROGRESS_ENABLED,
                           m_ui->common_progress_enabled_input->isChecked());
  Config::Save();
}
...
{
  m_ui->common_login_failed->setVisible(false);
  Config::SetBaseOrCurrent(Config::RA_USERNAME, m_ui->common_username_input->text().toStdString());
  AchievementManager::GetInstance().Login(m_ui->common_password_input->text().toStdString());
  m_ui->common_password_input->setText(QString());
  SaveSettings();
}
...
void AchievementSettingsWidget::ToggleHardcore()
{
  if (Config::Get(Config::RA_HARDCORE_ENABLED))
  {
    ...
    if (confirm != QMessageBox::Yes)
    {
      SignalBlocking(m_ui->common_hardcore_enabled_input)->setChecked(true);
      return;
    }
  }
  SaveSettings();
